...
def open_win():
    ...
    apt = Tk()
    apt.title("Interface")
    Label(apt, text="TSR MEDICAL STORE COMPANY").grid(row=0, column=0, columnspan=3)
...
def make_bill():
    ...
    m += "===============================================\n"
    m += "                                  No :%s\n\n" % det[5]
    m += " TSR MEDICAL STORE COMPANY\n"
    m += " JAIPUR BRANCH, RAJASTHAN\n\n"
    m += "-----------------------------------------------\n"
    if t == 1:
        m += "Name: %s\n" % named
        m += "Address: %s\n" % addd
...
def again():
    ...
    Label(root, text='TSR MEDICAL STORE COMPANY').grid(row=0, column=0, columnspan=5)
    Label(root, text="JAIPUR BRANCH, RAJASTHAN").grid(row=1, column=0, columnspan=5)
...
def open_cus():
    ...
    apt = Tk()
    apt.title("Interface")
    Label(apt, text="*** TSR MEDICAL DRUG STORE ***", bg='blue', fg='white').grid(row=0, column=0)
